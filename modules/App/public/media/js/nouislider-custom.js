        ...
        function tooltips ( ) {
            ...
            bindEvent('update', function(values, handleNumber, unencoded) {
				...
				if (typeof formattedValue == 'string') {
					formattedValue.replace(/^([0-9]{1,3})(?:[\.,]?([0-9]{3}))(?:[\.,]?([0-9]{3}))(?:[\.,]?([0-9]{3}))(?:(.,[0-9]+))$/g, '$1$2$3$4$5')
					.replace(/,/g, '.');
				}
				formattedValue = parseFloat(formattedValue).toFixed(2);
				formattedValue += "";
				var regex = /(\d)(\d{3})(\.\d{3})*(?:[\.,](\d\d))?$/g;
				while(formattedValue.match(regex)) {
					formattedValue = formattedValue.replace(regex, '$1.$2$3,$4');
				}

                tips[handleNumber].innerHTML = '<span>R$&nbsp;' + (formattedValue) + '</span>';
            });
        }

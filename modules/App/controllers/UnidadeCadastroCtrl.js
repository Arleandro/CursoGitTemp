angular.module('App')
	.controller('UnidadeCadastroCtrl', UnidadeCadastroCtrl);

UnidadeCadastroCtrl.$inject = [
	// angular
...
function UnidadeCadastroCtrl($scope, $state, $compile, $stateParams, AnuncioService, TipoComodidadeService, MapsService, AuthJwtStorage, FileUploader, undefined) {
	var self = this;
	...
	self.editing = $state.current.name == 'html.anunciosEditar';

	var today = new Date();
	today.setDate(today.getDate() + 90); // dias limite para o anuncio
	var expiration = today.toISOString();
	...
	self.anuncio = {
		...
		empreendimentoAnuncio: {codEstagioObra:1},
		unidadeAnuncios: [
			{
		...
		if (self.editing) {
			AnuncioService.put(anuncio)
				.success(function (data) {
					...
					$state.go('html.anunciosDetalhes', {idAnuncio: data[0].codAnuncio});
					app.hideLoad();
				})
				.error(function (cause) {
			...
			AnuncioService.post(anuncio)
				.success(function (data) {
					...
					$state.go('html.anunciosDetalhes', {idAnuncio: data.codAnuncio});
					app.hideLoad();
				})
				.error(function (cause) {
					var message = 'Erro gravando anúncio.';
	...
	self.positionMap = function()
	{
		...
		var address = '"' +
			...
			self.anuncio.enderecoAnuncio.nomComplemento + '"|"' +
			self.anuncio.enderecoAnuncio.sgUF + ' ' +
			self.anuncio.enderecoAnuncio.nomCidade + '"';
		...
		MapsService
			...
			.success(function (data) {
				...
				var message = cause;
				if (cause != undefined && cause != null && cause.error_message != undefined)
